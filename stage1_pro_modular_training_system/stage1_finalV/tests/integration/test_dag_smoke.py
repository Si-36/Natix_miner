...
from src.pipeline.registry import get_step_spec, resolve_execution_order
...
def test_export_calib_logits_step():
    ...
    # Create manifest
    run_id = "test_run_001"
    config = {"step": step_spec.__dict__()}
    ...
    assert ArtifactKey.VAL_CALIB_LOGITS.value in store._hashes, "Hash not recorded for VAL_CALIB_LOGITS"
    assert ArtifactKey.VAL_CALIB_LABELS.value in store._hashes, "Hash not recorded for VAL_CALIB_LABELS"
...
def test_dag_resolution():
    ...
    assert execution_order[0] == "export_calib_logits", \
        "First step should be export_calib_logits"
    assert execution_order[1] == "sweep_thresholds", \
        "Second step should be sweep_thresholds"
...
def test_manifest_tracking(run_id: str, store: ArtifactStore):
    ...
    assert ArtifactKey.THRESHOLDS_JSON.value in artifact_hashes, \
        "THRESHOLDS_JSON hash not recorded"
...
def test_split_contracts():
    ...
    assert Split.TRAIN not in export_allowed, "TRAIN in export_calib_logits allowed splits (LEAKAGE!)"
    assert Split.VAL_SELECT not in export_allowed, "VAL_SELECT in export_calib_logits allowed splits (LEAKAGE!)"
    ...
    assert Split.VAL_SELECT not in sweep_allowed, "VAL_SELECT in sweep_thresholds allowed splits (LEAKAGE!)"
    
    print(f"   ✅ sweep_thresholds split contract: VAL_CALIB ONLY (leak-proof)")
    
    # Verify VAL_TEST is NOT used (in this test)
    assert Split.VAL_TEST not in export_allowed, "VAL_TEST in export_calib_logits allowed splits (correct!)"
    assert Split.VAL_TEST not in sweep_allowed, "VAL_TEST in sweep_thresholds allowed splits (correct!)"
