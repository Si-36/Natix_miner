...
class TrainBaselineHeadSpec(StepSpec):
    ...
    def allowed_splits(cls) -> FrozenSet[str]:
        ...
        Returns:
            FrozenSet of allowed Split enums (as strings)
        """
    ...
    def run(self, ctx: StepContext) -> StepResult:
        ...
        freeze_backbone = (
            ...
            else cfg.training.freeze_backbone
        )
        ...
        learning_rate = (
            self.learning_rate if self.learning_rate is not None else cfg.training.learning_rate
        )
        ...
        ctx.artifact_store.put(
            ArtifactKey.VAL_SELECT_METRICS, str({"csv_path": str(metrics_path)}), ctx.run_id
        )
        ...
        manifest = (
            ctx.manifest
            if ctx.manifest
            else ctx.artifact_store.initialize_manifest(
                run_id=ctx.run_id,
                config=cfg,
            )
        )
